...
export const createInterviewSchema = z
  .object({
    ...
    interviewStyle: z
      .enum(
        ["MANGOS", "FAANG", "MAANG", "STARTUP", "CUSTOM"],
        "Interview style must be one of: MANGOS, FAANG, MAANG, STARTUP, CUSTOM",
      )
      .default("FAANG"),
    interviewType: z
      .enum(
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.duration < floor) {
      ctx.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["duration"],
        message: `Minimum duration for ${data.interviewType} interviews is ${floor} minutes`,
      });
    }
  });
