...
export function RegisterForm({ onSuccess, showModeLink = true }: { onSuccess: (email: string) => void; showModeLink?: boolean }) {
    ...
    return <div className="mx-auto w-full max-w-md"><h2 className="text-2xl font-semibold tracking-tight">Create your account</h2><p className="mt-1.5 text-sm text-muted-foreground">Start practicing adaptive AI interviews today.</p><form className="mt-6 flex flex-col gap-4" onSubmit={onSubmit} noValidate>
        ...
        <Button type="submit" className="mt-1 h-10 w-full" disabled={loading}>{loading ? <><Loader2 className="size-4 animate-spin" />Creating account…</> : 'Create account'}</Button>
    </form>{showModeLink ? <p className="mt-6 text-center text-xs text-muted-foreground">Already have an account? <Link to="/login" className="font-medium text-primary hover:underline">Sign in</Link></p> : null}</div>
}
